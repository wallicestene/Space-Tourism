...
function Destination() {
  ...
  return (
    <div className='md:bg-[url(/assets/destination/background-destination-desktop.jpg)] bg-[url(/assets/destination/background-destination-tablet.jpg)] bg-cover h-screen text-white bg-no-repeat overflow-scroll relative'>
      <div className='mt-5'>
        <Navbar/>
        <h1 className='text-center md:text-left md:ml-20 font-barlow text-xl tracking-widest uppercase mb-5'><span className=' text-gray-400'>01 </span>Pick Your Destination</h1>
      </div>
      <div className='grid md:grid-cols-2 grid-rows-1 gap-7  mx-10 md:place-items-center md:h-screen'>
      ...
      <div className='right md:w-4/5'>
        ...
        <div className='flex md:gap-8 gap-3 mb-5 flex-wrap justify-between md:justify-start' >
        {
          ...
          <p 
          className={`${index === value && "border-b-2 opacity-70 "} pb-1 hover:opacity-60 hover:cursor-pointer text-base mb-5 uppercase tracking-wider hover:border-b-2`}
          key={index} 
          onClick={() =>setvalue(index)}>{destination}</p>
          ))}
        </div>
        <h2 className='text-8xl mb-5 font-bellefair font-semibold'>{selectedDestination.name}</h2>
        <p className=' text-base md:10 mb-5'>{selectedDestination.description}</p>
        <hr />
